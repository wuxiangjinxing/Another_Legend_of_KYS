...
OEVENTLUA[1300] = function() --飞狐前代线
	if GetS(86,40,40,5) == 1 then
		addevent(-2, 2, 1, 667, 1)
		addevent(-2, 3, 1, 667, 1)
	else
	...
	say("好，干！",201,0,"酒客") 
	addevent(108, 12, 0,1301,3)--addevent(108, 12, 0, 1301, 3)
...
OEVENTLUA[1309] = function()	
	...
	if p1 == 1 and p2 == 1 then
		...
		if GetS(113,0,0,0) == 0 then
			say("另外我有一事还要相托少侠，请少侠万勿拒绝", 3)
			say("苗大侠请说，只要不违侠道，在下绝不推辞！")
			say("哈哈哈，不违的、不违的", 3)
			say("少侠可知，我曾以「金面佛苗人凤」之称行走江湖？", 3)
			say("自然知道，那可是「打遍天下无敌手」！")
			say("哈哈哈，那个称号当初有多响亮，现在就有多可笑", 3)
			say("但在那个称号之下，我的确打了许多场精彩的对决", 3)
			say("也是在那些对决中，我将佛道儒三家的精神融入剑法之中", 3)
			say("不敢说惊世骇俗，但我想我家传剑法的确有些奥妙了。", 3)
			say("而我之请求，便是为了感谢少侠", 3)
			say("希望少侠能听我一论「苗家天剑」之秘！", 3)
			if DrawStrBoxYesNo(-1, -1, "是否领悟苗家剑法？", C_WHITE, 30) == true then 
				say("苗大侠...在下恭敬不如从命！")
				dark()
				setLW1(44)
				light()
				QZXS("领悟苗家天剑！")
				say("哈哈哈，终于放下心中一块大石了！", 3)
			else
				say("...苗大侠是将在下当成图谋他人武学之人了？")
				say("在下之所以相助乃是感念二位侠义")
				say("因此只能心领苗大侠的好意，否则就是违背吾之侠义了！")
				say("哈哈哈！好，倒是苗某唐突了！", 3)
				say("这样吧，以后就叫我苗大哥，有甚困难，大哥必定出手助你！", 3)
				say("多谢苗大哥！")
			end
		end	
		dark()
...
OEVENTLUA[839] = function() --昆仑支线，救小妾
	if not checkpic(92, 1, 5284) then
		TalkEx("在下云游各方，途经崑仑，*见此三圣坳绿草如茵，*忍不住前来欣赏一番，*并顺道拜会人称*”铁琴先生”的何掌门．", 0, 1)  
